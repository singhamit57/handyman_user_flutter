                  ...
                  if (bookingData.bookingPackage != null)
                    PriceWidget(
                      price: calculateTotalAmount(
                        servicePrice: bookingData.bookingPackage!.price.validate(),
                      ...
                      children: [
                        if (bookingData.bookingType == BOOKING_TYPE_SERVICE)
                          PriceWidget(
                            ...
                            price: calculateTotalAmount(
                              servicePrice: (bookingData.isHourlyService && bookingData.status == BookingStatusKeys.complete)
                                  ? hourlyCalculationNew(
                                      price: bookingData.amount.validate(),
                                      secTime: bookingData.durationDiff.validate().toInt(),
                                    )
                                  : bookingData.amount.validate(),
                          ...
                          Row(
                            children: [
                              ...
                              PriceWidget(
                                price: calculateTotalAmount(
                                  serviceDiscountPercent: bookingData.discount.validate(),
                                  qty: bookingData.quantity.validate(value: 1).toInt(),
                                  servicePrice: bookingData.amount.validate(),
                                  taxes: bookingData.taxes,
                                  couponData: bookingData.couponData,
                                  extraCharges: bookingData.extraCharges.validate(),
                                ),
                                color: textSecondaryColorGlobal,
                                isBoldText: false,
                                size: textSecondarySizeGlobal,
                              ),
                              Text(language.hourly, style: secondaryTextStyle()).paddingSymmetric(horizontal: 4),
                            ],
                          )
                        else
                          4.width,
